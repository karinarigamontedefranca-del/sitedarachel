...
import { fetchImageForQuery } from "../../../../lib/unsplash";
    ...
    for (const title of newTopics) {
      try {
        ...
        const [img1, img2, img3, img4, img5] = await Promise.all([
          fetchImageForQuery(copy.imagem_busca_slide1),
          fetchImageForQuery(copy.imagem_busca_slide2),
          fetchImageForQuery(copy.imagem_busca_slide3),
          fetchImageForQuery(copy.imagem_busca_slide4),
          fetchImageForQuery(copy.imagem_busca_slide5),
        ]);

        const row = SHEET_COLUMNS.map((col) => {
          if (col === "trending_topic") return title;
          if (col === "status") return "pendente";
          if (col === "slide1_imagem") return img1;
          if (col === "slide2_imagem") return img2;
          if (col === "slide3_imagem") return img3;
          if (col === "slide4_imagem") return img4;
          if (col === "slide5_imagem") return img5;
