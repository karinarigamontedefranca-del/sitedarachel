export type CarouselCopy = {
  ...
  imagem_busca_slide1: string;
  imagem_busca_slide2: string;
  imagem_busca_slide3: string;
  imagem_busca_slide4: string;
  imagem_busca_slide5: string;
};
...
- imagem_busca_slide1 até imagem_busca_slide5: uma descrição visual EM INGLÊS (3 a 6
  palavras) para CADA slide, ligada especificamente ao que aquele slide individual diz
  — não repita a mesma descrição nos 5 campos. Deve ser algo concreto e fotografável
  que exista como foto de banco de imagens (ex: "neon city skyline night", "sports car
  street night", "gaming setup dark room", "marketing team brainstorm office",
  "smartphone social media notification"). Pense na estética/atmosfera do assunto, não
  no assunto literal.
  REGRAS OBRIGATÓRIAS: nunca inclua nomes de jogos, filmes, séries, marcas, produtos,
  empresas, pessoas reais ou personagens — mesmo que o trending topic seja sobre algo
  específico (ex: um jogo, um filme, uma marca), descreva apenas a cena/objeto/atmosfera
  genérica que evoca aquele universo, nunca a propriedade intelectual em si.

Responda APENAS com um objeto JSON válido, sem markdown, sem texto antes ou depois,
com exatamente essas 12 chaves.`;
